...
(function () {
  ...
  function orderConcernCompanies(results) {
    const concernCompanies = results.filter((company) =>
      company.companyCode.endsWith("K"),
    );
    ...
    const regularCompanies = results.filter(
      (company) =>
        !concernCodeSet.has(company.companyCode) &&
        !company.companyCode.endsWith("K"),
    );
  ...
  function normalizeSearchResponse(json) {
    const hits =
      json && json.hits && Array.isArray(json.hits.hit) ? json.hits.hit : [];
    ...
    const results = hits
      ...
      .filter(
        (result) =>
          result.followedModelId && result.companyName && result.nameSlug,
      );
  ...
  async function fetchSearchResults(query, signal) {
    ...
    const response = await fetch(
      `${SEARCH_ENDPOINT}/search?q=${encodedQuery}`,
      { signal },
    );
  ...
  function buildResultLink(result, index, dropdownId) {
    ...
    link.addEventListener("click", () => {
      if (typeof window.gtag === "function") {
        window.gtag("event", "company_result_click", {
          company_id: result.followedModelId,
          search_surface: window.location.pathname.startsWith("/da/guides/")
            ? "guide"
            : "marketing",
        });
      }
  ...
  function buildBuyResultButton(result, index, dropdownId) {
    ...
    button.addEventListener("click", () => {
      if (typeof window.gtag === "function") {
        window.gtag("event", "report_cta_click", {
          report_type: "dk_ai_credit_risk",
          company_id: result.followedModelId,
          search_surface: "marketing_order",
        });
      }
        ...
        if (shouldRenderForCurrentFocus(form)) {
          renderMessage(
            ...
            localized(
              ...
              "Søgningen er midlertidigt utilgængelig",
            ),
          );
        }
      } finally {
  ...
  function initSearchForm(form, index) {
    ...
    const dropdown = createSearchDropdown(
      `company-search-results-${index + 1}`,
    );
